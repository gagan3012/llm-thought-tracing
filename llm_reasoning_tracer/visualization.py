...
def animate_reasoning_flow(
    ...
    ax.set_title(
        f"Prompt: {path_results['prompt']} Reasoning Path: {' → '.join(path_results['best_path'])}",
        fontsize=12,
        pad=15,
    )
    ...
    def animate(frame_idx):
        if frame_idx < len(positions):
            ...
            label = ax.text(
                positions[frame_idx],
                compressed_layers[frame_idx] + 0.3,
                concepts[frame_idx],
                fontsize=9,
                ha="center",
                va="bottom",
                color="black",
                bbox=dict(
                    boxstyle="round,pad=0.3", fc=text_bg_color, ec="gray", alpha=0.9
                ),
                zorder=6,
            )
            labels.append(label)
...
def animate_reasoning_flow_dark(
    ...
    and prevents arrows from overflowing the image boundaries.
    """
    ...
    # Add some padding to the plot boundaries to accommodate arrows
    x_padding = 0.8
    y_padding = 0.8

    # Set plot limits with padding
    ax.set_xlim(0.5 - x_padding, len(tokens) + 0.5 + x_padding)
    ax.set_ylim(-0.5 - y_padding, compressed_max_layer - 0.5 + y_padding)

    ax.set_xlabel("Token (Prompt)", fontsize=10, color="white", labelpad=8)
    ax.set_ylabel(
        "Transformer Layers (Compressed)", fontsize=10, color="white", labelpad=8
    )
    ax.set_title(
        f"Prompt: {path_results['prompt']} Reasoning Path: {' → '.join(path_results['best_path'])}",
        fontsize=13,
    ...
    # Use different colors for regular and final concept
    ...
    def animate(frame_idx):
        ...
        if frame_idx < len(positions):
            ...
            label = ax.text(
                positions[frame_idx],
                compressed_layers[frame_idx] + 0.2,
                concepts[frame_idx],
                fontsize=font_size,
                ha="center",
                va="bottom",
            ...
            if idx + 1 < len(positions):
                ...
                # Dynamically adjust the curvature based on distance - lower for larger distances
                # This helps keep arrows within plot boundaries
